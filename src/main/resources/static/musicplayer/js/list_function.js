	...
	$.ajax({
		...
		success:function(data){
			...
			for(var vals of NECsongs){
				...
				html += `<div class="list-item" data-url="${vals.musicUrl}" data-pic="${vals.picUrl}" data-lrc="${vals.lrc}">
	                    <span class="list-num">${count}</span>
	                    <span class="list-mobile-menu"></span>
	                     <!--  <span class="music-album">歌曲时间</span> -->
	                    <span class="auth-name">${vals.author}</span>
	                    <span class="music-name">${vals.musicName}</span>
	                </div>`;
                count++;
			}
